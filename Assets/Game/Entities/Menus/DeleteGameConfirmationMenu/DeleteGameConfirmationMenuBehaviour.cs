...
namespace EmergenciaQuimica {
	...
	public sealed class DeleteGameConfirmationMenuBehaviour : MenuBehaviour {
		...
		public override void OnShowGui() {
			...
			Framework.BeginArea(0.25f, 0.35f, 0.5f, 0.3f); {
				Framework.BeginArea(0f, 0f, 1f, 0.6f); {
					GUILayout.Box(Framework.GetText("DeleteGameConfirmationMenuBox"), menuBoxStyle, GUILayout.ExpandHeight(true));
				} Framework.EndArea();

				Framework.BeginArea(0f, 0.7f, 1f, 0.3f); {
					Framework.BeginArea(0f, 0f, 0.49f, 1f); {
						if (GUILayout.Button(Framework.GetText("DeleteGameConfirmationMenuCancelButton"), menuButtonStyle))
							OnCancel();
					} Framework.EndArea();

					Framework.BeginArea(0.51f, 0f, 0.49f, 1f); {
						if (GUILayout.Button(Framework.GetText("DeleteGameConfirmationMenuDeleteGameButton"), menuButtonStyle))
							OnDeleteGame();
					} Framework.EndArea();
				} Framework.EndArea();
			} Framework.EndArea();
