	...
	public static partial class StateManager {
		...
		private static XDocument GenerateStateFile() {
			...
			foreach (string hint in hints) {
				// Sets the hint
				XElement hintNode = new XElement(ParameterManager.XmlTagHint);
				UtilityManager.SetXmlNodeStringValue(hintNode, hint);
		...
		private static void ProcessStateFile(XDocument stateFile) {
			...
			// Hints
			foreach (XElement hintNode in stateNode.Elements(ParameterManager.XmlTagHint)) {
				// Gets the hint
				string hint = UtilityManager.GetXmlNodeStringValue(hintNode);
