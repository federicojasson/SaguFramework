...
namespace SaguFramework {
	...
	public static class CreationManager {
		...
		public static void CreateItem(ItemParameters itemParameters, Vector2 positionInGame, float scaleFactor) {
			...
			GameObject imageObject = new GameObject();
			UtilityManager.SetParent(imageObject, itemObject);

			GameObject interactiveEntityObject = new GameObject();
			UtilityManager.SetParent(interactiveEntityObject, itemObject);
		...
		public static void CreatePlayerCharacter(CharacterParameters characterParameters, Vector2 positionInGame, float scaleFactor) {
			...
			// TODO: test
			BoxCollider2D collider = playerCharacterObject.AddComponent<BoxCollider2D>();
			collider.center = image.transform.position;
			collider.size = new Vector2(image.GetWidthUnits(), image.GetHeightUnits());
